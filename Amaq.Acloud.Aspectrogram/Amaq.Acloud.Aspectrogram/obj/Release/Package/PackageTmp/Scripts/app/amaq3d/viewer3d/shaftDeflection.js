...
ShaftDeflection = function () {
    ...
    ShaftDeflection = function (idEntity, wId) {
        ...
        _getPoints = function () {
            ...
            _isPrincipal = ej.DataManager(mainCache.loadedAssets).executeLocal(new ej.Query().where("Id", "equal", idEntity, false))[0].IsPrincipal;
            ...
            for (var i = 0; i < nodes[idEntity + wId].Properties3d.points.children.length; i++) {
                ...
                if (nodes[idEntity + wId].Properties3d.points.children[i].indexPiece.hasOwnProperty(idEntity)) {
                    indexPiece = nodes[idEntity + wId].Properties3d.points.children[i].indexPiece[idEntity];
                }
                else if (nodes[idEntity + wId].Properties3d.points.children[i].indexPiece.hasOwnProperty("level13d")) {
                    if (_isPrincipal) {
                        indexPiece = nodes[idEntity + wId].Properties3d.points.children[i].indexPiece.level13d;
                    } else {
                        indexPiece = nodes[idEntity + wId].Properties3d.points.children[i].indexPiece.level23d;
                    }
                }
