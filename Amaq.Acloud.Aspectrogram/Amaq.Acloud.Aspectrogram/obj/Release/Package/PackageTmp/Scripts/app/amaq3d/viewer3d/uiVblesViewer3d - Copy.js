...
UiVblesViewer3d = (function () {
    ...
    UiVblesViewer3d = function (idEntity, wId) {
        ...
        this.createVblesWindows = function(){

            _createGroups();
            _createFilterModal();
            $("#filterModal-" + idEntity + wId).hide();
            $("#filterViewer-" + idEntity + wId).on("click", function (args) {
                if ($("#filterModal-" + idEntity + wId)[0].style.display == "none") {
                    $("#filterModal-" + idEntity + wId).show();
                }
                else {
                    $("#filterModal-" + idEntity + wId).hide();
                }
            });
            $("#sortingViewer-" + idEntity + wId).on("click", function (args) {
                /*if ($("#sortingModal-" + idEntity + wId)[0].style.display == "none") {
                    $("#sortingModal-" + idEntity + wId).show();
                }
                else {
                    $("#sortingModal-" + idEntity + wId).hide();
                }*/
            });
        };

        _createFilterModal = function () {

            var qtyGroups = 3, i, j;
            //filterViewer-' + idEntity + wId 

            _containerId = viewer3d.containerCanvas[idEntity + wId].id;
            _container = $("#" + _containerId);

            _container.append('<div id="filterModal-' + idEntity + wId + '"></div>');
            $("#filterModal-" + idEntity + wId).addClass("classDivFilter");

            for (i = 0; i < qtyGroups; i++) {
                $("#filterModal-" + idEntity + wId).append('<div id="filterModal-Group-' + i + "-" + idEntity + wId + '"></div>');
                $("#filterModal-Group-" + i + "-" + idEntity + wId).addClass("groupsFilter");
                for (j = 0; j < _groupsFilter[i].group.length; j++) {

                    if (j === 0) {
                        $("#filterModal-Group-" + i + "-" + idEntity + wId).append('<input type="radio" name="' + _groupsFilter[i].name + idEntity + wId + '" value="' + _groupsFilter[i].group[j] + '" checked class="radioButtonFilter">' + _groupsFilter[i].group[j] + '<br>');
                    }
                    else {
                        $("#filterModal-Group-" + i + "-" + idEntity + wId).append('<input type="radio" name="' + _groupsFilter[i].name + idEntity + wId + '" value="' + _groupsFilter[i].group[j] + '" class="radioButtonFilter">' + _groupsFilter[i].group[j] + '<br>');
                    }                  
                }
            ...
            $(document).on('click', '[name="' + _groupsFilter[0].name + idEntity + wId + '"]', function (args) {
                _selectedVarMd = args.target.value;
                _aplicateFilter();
            });

            $(document).on('click', '[name="' + _groupsFilter[1].name + idEntity + wId + '"]', function (args) {
                _selectedStatus = args.target.value;
                _aplicateFilter();
            });

            $(document).on('click', '[name="' + _groupsFilter[2].name + idEntity + wId + '"]', function (args) {
                console.log("jsjsjs");
                _selectedSubVar = args.target.value;
                _aplicateFilter();
            });
