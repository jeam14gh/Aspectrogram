...
UiWaterfall3d = (function () {
     ...
     * Constructor.
     */
    UiWaterfall3d = function (idPoint, waterfall, wId, arraySignal, arraySignalY, flagFullSpec) {
            ...
            this.saveInfoWaterfall = function () {
                ...
                if (flagFullSpec) {
                    if (fSWatConfig.type == "espectrograma") {
                        waterfall.fillWaterfallHistCubesFullSpec(arraySignal, arraySignalY);
                    }
                    else
                   {
                        waterfall.fillClassicWaterfallHistFullSpec(arraySignal, arraySignalY);
                    }
                } else {
        ...
        this.modalPersonalize = {
            ...
            children: [
                ...
                },/*
                {
                    ...
                    obj: null,
                    name: "InQtySpecWat-",
                    txt: "Cantidad Espectros: ",
                    type: "number",
                    fcn: _asignQtySpec,
                    event: "change",
                    className: "InNum-Waterfall",
                    value: _values.specQty,
                    max: 500
                },
                {
                    obj: null,
                    name: "InQtyArmWat-",
                    txt: "Cantidad Armónicos: ",
                    type: "number",
                    fcn: _asignQtyArm,
                    event: "change",
                    className: "InNum-Waterfall",
                    value: _values.armQty,
                    max: 15
                },*/
                {
                    obj: null,
                ...
                }, /*             
                {
                    ...
                    txt: "Aplicar ",
                    type: "button",
                    fcn: _saveInfoWaterfall,
                    event: "click",
                    className: "InBtn-Waterfall"
                },
                {
                    obj: null,
                    name: "BtnCancelWat-",
        ...
        //InBtn-Waterfall - InColor-Waterfall - ImgBmp-Waterfall - InCB-Waterfall - InNum-Waterfall
        //number - button - colorPicker - img - checkbox
        /*
        _createModalConfig = function () {

            var event;

            _container.append('<div id="' + _modalPersonalize.id + "-" + idPoint + wId + '"></div>');
            _modalPersonalize.obj = $("#" + _modalPersonalize.id + "-" + idPoint + wId);

            _modalPersonalize.obj.addClass("infoGralWaterfall3d modalPersonalize-Waterfall3d-");
            _modalPersonalize.obj.css({
                "width": "240px",
            ...
            for (var i = 0; i < _modalPersonalize.children.length; i++) {
                
                switch (_modalPersonalize.children[i].type) {
                    case "number":
                    case "color":
                    case "checkbox":
                    case "range":

                        _modalPersonalize.obj.append('<div style="display: inline-block; position: relative; padding: 25px 15px 5px 15px;">' + _modalPersonalize.children[i].txt + '</div>');
                        _modalPersonalize.obj.append('<input id="' + _modalPersonalize.children[i].name + idPoint + wId + '" type="' + _modalPersonalize.children[i].type + '" name="' + _modalPersonalize.children[i].name + '" min="1" value="' + _modalPersonalize.children[i].value + '">');

                        _modalPersonalize.children[i].obj = $("#" + _modalPersonalize.children[i].name + idPoint + wId);
                        
                        _modalPersonalize.children[i].obj.addClass(_modalPersonalize.children[i].className);

                        if (_modalPersonalize.children[i].type === "checkbox") {
                            //_modalPersonalize.children[i].obj.checked
                            _modalPersonalize.children[i].obj.prop('checked', _modalPersonalize.children[i].checked);
                        }
                        if (_modalPersonalize.children[i].type === "number" || _modalPersonalize.children[i].type === "range") {
                            _modalPersonalize.children[i].obj.prop('max', _modalPersonalize.children[i].max);
                        }

                        break;

                    case "img":
                        _modalPersonalize.obj.append('<img id="' + _modalPersonalize.children[i].name + idPoint + wId + '" src="' + _modalPersonalize.children[i].img + '">');
                        _modalPersonalize.children[i].obj = $("#" + _modalPersonalize.children[i].name + idPoint + wId);
                        _modalPersonalize.children[i].obj.addClass(_modalPersonalize.children[i].className);
                        break;
                    case "button":
                        _modalPersonalize.obj.append('<input id="' + _modalPersonalize.children[i].name + idPoint + wId + '" type="' + _modalPersonalize.children[i].type + '" name="' + _modalPersonalize.children[i].name + '" value="' + _modalPersonalize.children[i].txt + '">');
                        _modalPersonalize.children[i].obj = $("#" + _modalPersonalize.children[i].name + idPoint + wId);
                        _modalPersonalize.children[i].obj.addClass(_modalPersonalize.children[i].className);
                        break;
                }
                
                event = _modalPersonalize.children[i].event;

                if (event !== null) {
                    _modalPersonalize.children[i].obj.on(event, function (args) {
                        _value = args.target.value;
                        _chooseFunction(args.currentTarget.id, "ModalConfig");
                        //_modalPersonalize.children[i].value = args.target.value;
            ...
            if (watConfig.type == "espectrograma") {
                $("#InClassicWat-" + idPoint + wId).prop('checked', false);
                $("#InSpectrogramWat-" + idPoint + wId).prop('checked', true);
            }
            else {
                $("#InClassicWat-" + idPoint + wId).prop('checked', true);
                $("#InSpectrogramWat-" + idPoint + wId).prop('checked', false);
            }
        ...
        */
        ...
        _createGralInfo = function () {
            ...
            $("#" + scope.gralInfo.id + "-" + idPoint + wId).append('<div id="' + scope.gralInfo.pointInfo.id + "-"
                + idPoint + wId + '"></div>');
            //$("#" + scope.gralInfo.pointInfo.id + "-" + idPoint).addClass(scope.gralInfo.pointInfo.className);
            ...
            //$("#" + scope.gralInfo.pointInfo.id + "-" + idPoint).append('</br><div><strong><u> ARMÓNICOS </u></strong></div></br>');
            ...
            for (var i = 0; i < waterfall.qtyArm; i++) {
                ...
                //console.log(waterfall.armonicColors);
            ...
            $("#" + scope.gralInfo.frequency.id + "-" + idPoint + wId).append('<input type="number" id="' + scope.gralInfo.frequency.parts[1] + "-" + idPoint + wId + '" value="10" min="1">');
            ...
            $("#" + scope.gralInfo.frequency.id + "-" + idPoint + wId).append('<input type="range" id="' + scope.gralInfo.frequency.parts[2] + "-" + idPoint + wId + '" value="10" min="1">');
            ...
            $("#" + scope.gralInfo.frequency.parts[1] + "-" + idPoint + wId).on("change", function (args) {
                if($("#" + scope.gralInfo.frequency.parts[0] + "-" + idPoint + wId).val() =="on"){
                    checked = true;
                }else{
                    checked = false;
                }
                $("#" + scope.gralInfo.frequency.parts[2] + "-" + idPoint + wId).val($("#" + scope.gralInfo.frequency.parts[1] + "-" + idPoint + wId).val());
                waterfall.chooseFrecuency($("#" + scope.gralInfo.frequency.parts[2] + "-" + idPoint + wId).val(), checked);
            });
            ...
            $("#" + scope.gralInfo.frequency.parts[2] + "-" + idPoint + wId).on("change", function (args) {
                ...
                $("#" + scope.gralInfo.frequency.parts[1] + "-" + idPoint + wId).val($("#" + scope.gralInfo.frequency.parts[2] + "-" + idPoint + wId).val());
                waterfall.chooseFrecuency($("#" + scope.gralInfo.frequency.parts[2] + "-" + idPoint + wId).val(), checked);
            });
        ...
        this.actualizeGralInfo = function (flagReload) {
            ...
            //frequency
            //scope.gralInfo.frequency.parts[0] + "-" + idPoint + wId + 
            //$("#" + scope.gralInfo.armonics.name + i + "-" + scope.gralInfo.armonics.parts[1] + "-" + 1 + "-" + idPoint + wId).attr("checked", false);
            ...
            if (flagFullSpec) {
                ...
                waterfall.createExtraArmonicLine();
            }
